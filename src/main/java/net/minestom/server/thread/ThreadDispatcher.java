 ...
 * contain {@link Tickable} constituents such as entities. The parallelism of such updates is defined when the
 * dispatcher is constructed.
 *
 * @see ThreadDispatcher#of(ThreadProvider, int)
 * @see ThreadDispatcher#of(ThreadProvider, IntFunction, int)
 * @see ThreadDispatcher#singleThread()
 */
public final class ThreadDispatcher<P> {
    ...
    /**
     * Creates a new ThreadDispatcher using default thread names.
     *
     * @param provider the {@link ThreadProvider} instance to be used for defining the thread IDs
     * @param threadCount the number of threads to be used by this dispatcher
    ...
    /**
     * Creates a new ThreadDispatcher. Each tick thread should have a unique name, which will be generated by the
     * provided {@link IntFunction}.
     *
     * @param provider the {@link ThreadProvider} instance to be used for defining the thread IDs
     * @param nameGenerator an int-accepting function used for generating unique names
     * @param threadCount the number of threads to be used by this dispatcher
     * @return a new ThreadDispatcher instance
     * @param <P> the dispatcher partition type
     */
    ...
    /**
     * Creates a new single-threaded dispatcher which uses default thread names.
     *
     * @return a single-threaded dispatcher
     * @param <P> the dispatcher partition type
     */
    ...
    public synchronized void updateAndAwait(long time) {
        ...
        this.updates.drain(update -> {
            ...
            } else if (update instanceof DispatchUpdate.ElementRemove<P> elementRemove) {
    ...
    /**
     * Called at the end of each tick to clear removed tickables, refresh the partition linked to a tickable, and group
     * threads based on {@link ThreadProvider#findThread(Object)}.
     *
    ...
    private void processUpdatedElement(Tickable tickable, P partition) {
        ...
        if (partitionEntry != null) {
            ...
            if (tickable instanceof AcquirableSource<?> source) {
